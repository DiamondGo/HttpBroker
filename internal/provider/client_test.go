...
// keep polling without erroring, so a test can capture what the provider
...
func TestClientPrefersStreamModeByDefault(t *testing.T) {
	...
	if !req.PreferStreamMode {
		t.Fatal("provider's connect request did not set prefer_stream_mode — default should prefer stream")
	}
...
func TestClientPollModeBatchDisablesPreferStream(t *testing.T) {
	...
	req, _ := fb.firstConnect()
	if req.PreferStreamMode {
		t.Fatal("provider's connect request set prefer_stream_mode with PollMode: \"batch\" configured")
	}
...
func waitForFirstConnect(t *testing.T, fb *fakeBroker) {
	...
	for {
		...
		if time.Now().After(deadline) {
			t.Fatal("timed out waiting for the provider to connect")
		}
